# L'idée est de construire le graphe au fur et à mesure un peu comme dans un algorithme de Dijkstra en prenant à chaque fois le chemin de plus petit poids qui relie un point déjà dans le graphe à un qui n'y est pas encore.
...
# Cherchons 
def chercher():
    # Commençons par créer un heap queue qui aura comme élément (valeur,(numéro du premier point, numéro du second point)) où valeur est le poids du chemin reliant le premier point au second.
    heap=[]
    hq.heapify(heap)
    # Cherchons le poids minimum dans notre matrice
    p_min=inf
    i_min,j_min=0,0
    # on en profite pour calculer la somme des poids des chemins du graphe
    somme_totale=0
    for i in range(N):
        for j in range(i+1):
            temp = matrice[i][j]
            if temp!="-":
                ...
                if temp <p_min :
                    p_min=temp
                    i_min,j_min=i,j
    # Somme des valeurs de notre nouveau graphe
    somme=p_min
    # Points déjà dans notre nouveau graphe
    closed=set([i_min,j_min])
    # On rajoute à notre heap queue les points reliés à i_min et j_min
    for k in range(N):
        if matrice[i_min][k]!="-":
            hq.heappush(heap,(matrice[i_min][k],(i_min,k)))
        if  matrice[j_min][k]!="-":
            hq.heappush(heap,(matrice[j_min][k],(j_min,k)))
    while len(closed)!=N: # tant qu'on a pas regardé tous les sommets
        val,(p1,p2)=hq.heappop(heap)
        if p2 in closed : continue # Par construction, p1 est déjà dans closed. Si p2 l'est aussi, on a déjà un chemin reliant ces points donc on peut passer au suivant
        # Si p2 n'est pas encore dans le graphe, on le rajoute et on rajoute tous les chemins le concernant dans la heap des chemins à regarder
        closed.add(p2)
        somme+=val
        for k in range(N):
            if matrice[p2][k]!="-":
                hq.heappush(heap,(matrice[p2][k],(p2,k)))
    return somme_totale-somme
